...
namespace Openpay\Codi\Observer;

use Magento\Framework\Event\Observer;
use Magento\Framework\Event\ObserverInterface;
use Openpay\Codi\Model\Payment as Config;
...
class AfterPlaceOrder implements ObserverInterface {
    ...
    public function execute(Observer $observer) {
        ...
        } elseif($order->getPayment()->getMethod() == 'openpay_codi') {
            $this->logger->debug('#AfterPlaceOrder openpay_codi', array('order_id' => $orderId[0], 'order_status' => $order->getStatus(), 'ext_order_id' => $order->getExtOrderId()));

            if ($order->getStatus() == 'pending' && isset($_SESSION['openpay_codi_redirect_url'])) {
                $this->logger->debug('#AfterPlaceOrder openpay_codi', array('ext_order_id' => $order->getExtOrderId(), 'redirect_url' => $_SESSION['openpay_codi_redirect_url']));
                $this->_actionFlag->set('', \Magento\Framework\App\Action\Action::FLAG_NO_DISPATCH, true);
                $this->_redirect->redirect($this->_response, $_SESSION['openpay_codi_redirect_url']);
            }
        }
